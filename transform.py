...
def apply_brightness(image, factor): # Renomeando 'brighten' para 'apply_brightness'
...
def apply_contrast(image, factor, mid): # Renomeando 'adjust_contrast' para 'apply_contrast'
...
def apply_blur(image, kernel_size): # Renomeando 'blur' para 'apply_blur'
...
if __name__ == '__main__':
    ...
    # brightening
    brightened_im = apply_brightness(lake, 1.7)
    brightened_im.write_image('brightened.png')

    # darkening
    darkened_im = apply_brightness(lake, 0.3)
    darkened_im.write_image('darkened.png')

    # increase contrast
    incr_contrast = apply_contrast(lake, 2, 0.5)
    incr_contrast.write_image('increased_contrast.png')

    # decrease contrast
    decr_contrast = apply_contrast(lake, 0.5, 0.5)
    decr_contrast.write_image('decreased_contrast.png')

    # blur using kernel 3
    blur_3 = apply_blur(city, 3)
    blur_3.write_image('blur_k3.png')

    # blur using kernel size of 15
    blur_15 = apply_blur(city, 15)
    ...
    img_edges = detect_edges(city) # Executando a função detect_edges
    img_edges.write_image('edge_xy.png') 